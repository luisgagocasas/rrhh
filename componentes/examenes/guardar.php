...
if (!empty($_FILES['archivo']['name'])) {
    if ($_FILES['archivo']["error"] > 0){ $respuesta->mensaje .= "Error: ".$_FILES['archivo']['error']."<br />"; }
        ...
        $nombreft = $_POST['iduser']."_".$_POST['idexamen']."_".LGlobal::Url_Amigable($_POST['nombreuser'].$_POST['apellidopuser'].$_POST['apellidomuser']).".".$tipoft;
        $fotoruta = "../../archivos_examenes/";
        move_uploaded_file($_FILES['archivo']['tmp_name'],$fotoruta.$nombreft);
    }
}
else { $nombreft = $_POST['archivo']; }
if($_POST['tipo']=="crear"){
    $sql = "INSERT INTO com_examen_asignar (id_examen, id_usuario, fechainicio, fechafin, archivo, activo) VALUES ('".$_POST['idexamen']."', '".$_POST['iduser']."', '".$_POST['inicio']."', '".$_POST['fin']."', '".$nombreft."', '".$_POST['estado']."')";
    mysql_query($sql,$con);
    $respuesta->mensaje .= "
    <script>
...
if($_POST['tipo']=="editar"){
    $sql = "UPDATE com_examen_asignar SET id_examen='".$_POST['idexamen']."', id_usuario='".$_POST['iduser']."', fechainicio='".$_POST['inicio']."', fechafin='".$_POST['fin']."', archivo='".$nombreft."', activo='".$_POST['estado']."' WHERE id_asig_examen='".$_POST['idasig']."'";
    mysql_query($sql,$con);
    $respuesta->mensaje .= "
    <script>
    $(\".cerrar\").click(function(){
...
if($_GET['tipo']=="borrararchivo"){
    $sql = "UPDATE com_examen_asignar SET archivo='' WHERE id_asig_examen='".$_GET['id']."'";
    mysql_query($sql,$con);
    unlink("../../archivos_examenes/".$fotoruta.$_GET['archivo']);
    $respuesta->mensaje .= "
    <script>
    $(\".cerrar\").click(function(){
        $(\".jo\").addClass('bounceOutUp');
