...
else if($_GET['que']=="sedeexamen"){
		$salida_cvs = "Sede:,".$_GET['sede'].",";
		$salida_cvs .= "\n";
		$salida_cvs .= utf8_decode("Exámenes").":".",".Reporte::nombresedesexamen($_GET['idsede'], true).",";
		$salida_cvs .= "\n";
	$respasig = mysql_query("SELECT * from com_examen_asignar where id_examen=".$_GET['idsede']."");
	$respcursos = mysql_query("select * from com_examenes where sede_id='".$_GET['idsede']."'");
	$usuarios = mysql_query('select * from usuarios');
		$salida_cvs .= mysql_field_name($usuarios, 12).",";
		$salida_cvs .= mysql_field_name($usuarios, 4).",";
		$salida_cvs .= mysql_field_name($usuarios, 5).",";
		$salida_cvs .= mysql_field_name($usuarios, 6).",";
		$salida_cvs .= mysql_field_name($respcursos, 1).",";
		$salida_cvs .= mysql_field_name($respcursos, 2).",";
		$salida_cvs .= mysql_field_name($respasig, 3).",";
		$salida_cvs .= mysql_field_name($respasig, 4)."";
		$salida_cvs .= "\n";
	while($examenes = mysql_fetch_array($respcursos)){
		$respasig1 = mysql_query("SELECT * from com_examen_asignar where id_examen=".$examenes['examen_id']."");
		while($asigcurso = mysql_fetch_array($respasig1)){
			$user = mysql_query("SELECT * FROM usuarios where id=".$asigcurso['id_usuario']."");
			$rowr = mysql_fetch_array($user);
			$salida_cvs .= $rowr['dni'].", ";
			...
			$salida_cvs .= utf8_decode($rowr['apellidop']).", ";
			$salida_cvs .= utf8_decode($rowr['apellidom']).", ";
			$salida_cvs .= utf8_decode($examenes['examen_nombre']).", ";
			$salida_cvs .= utf8_decode(Reporte::nombreclinica($examenes['id_clinica'])).", ";
			$salida_cvs .= $asigcurso['fechainicio'].", ";
			$salida_cvs .= $asigcurso['fechafin']."";
			$salida_cvs .= "\n";
		}
...
else if($_GET['que']=="seguro"){
		$salida_cvs = "Sede:,".$_GET['sede'].",";
		$salida_cvs .= "\n";
		$salida_cvs .= utf8_decode("Seguro").":".",".$_GET['seguro'].",";
		$salida_cvs .= "\n";
	$respasig = mysql_query("SELECT * from com_seguro_asignar where id_seguro=".$_GET['idseguro']."");
	$usuarios = mysql_query('select * from usuarios');
		$salida_cvs .= mysql_field_name($usuarios, 12).",";
		$salida_cvs .= mysql_field_name($usuarios, 4).",";
		$salida_cvs .= mysql_field_name($usuarios, 5).",";
...
else if($_GET['que']=="sedeseguro"){
		$salida_cvs = "Sede:,".$_GET['sede'].",";
		$salida_cvs .= "\n";
		$salida_cvs .= utf8_decode("Seguros").":".",".Reporte::nombresedeseguro($_GET['idsede'], true).",";
		$salida_cvs .= "\n";
	$respasig = mysql_query("SELECT * from com_seguro_asignar where id_seguro=".$_GET['idsede']."");
	$respcursos = mysql_query("select * from com_seguros where sede_id='".$_GET['idsede']."'");
	$usuarios = mysql_query('select * from usuarios');
		$salida_cvs .= mysql_field_name($usuarios, 12).",";
		$salida_cvs .= mysql_field_name($usuarios, 4).",";
		$salida_cvs .= mysql_field_name($usuarios, 5).",";
		...
		$salida_cvs .= mysql_field_name($respasig, 4)."";
		$salida_cvs .= "\n";
	while($seguro = mysql_fetch_array($respcursos)){
		$respasig1 = mysql_query("SELECT * from com_seguro_asignar where id_seguro=".$seguro['seguro_id']."");
		while($asigcurso = mysql_fetch_array($respasig1)){
			$user = mysql_query("SELECT * FROM usuarios where id=".$asigcurso['id_usuario']."");
			$rowr = mysql_fetch_array($user);
			$salida_cvs .= $rowr['dni'].", ";
			...
			$salida_cvs .= utf8_decode($rowr['apellidop']).", ";
			$salida_cvs .= utf8_decode($rowr['apellidom']).", ";
			$salida_cvs .= utf8_decode($seguro['seguro_nombre']).", ";
			$salida_cvs .= $asigcurso['fechainicio'].", ";
			$salida_cvs .= $asigcurso['fechafin']."";
			$salida_cvs .= "\n";
		}
